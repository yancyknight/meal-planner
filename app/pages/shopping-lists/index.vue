    ...
    <Teleport to="body">
      ...
      >
        <div class="w-full max-w-md rounded-xl border border-border bg-surface p-6 shadow-xl">
          ...
          <div class="space-y-4">
            <div>
              <label class="mb-1 block text-xs font-medium uppercase tracking-wider text-text-muted">Name</label>
              <input
                v-model="form.name"
                type="text"
                placeholder="e.g. Week of May 19"
                class="w-full rounded-lg border border-border bg-bg px-3 py-2 text-sm text-text placeholder:text-text-subtle focus:outline-none focus:ring-2 focus:ring-accent/40"
              />
            </div>
...
const { mutateAsync: createList, isPending: creating } = useMutation({
  mutationFn: (body: { name: string; dateRangeStart: string; dateRangeEnd: string }) =>
